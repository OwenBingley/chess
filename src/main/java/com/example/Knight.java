...
int[] signs = { 1, -1 }; // This is used later in for each loops to make it
//so the knight moves in
// positive and negative directions both
//horizontally and vertically
int[][] pairs = { { 1, 2 }, { 2, 1 } }; // Knights move in an "L-shape"
//patter with either moves that are 2 up
// and 1 to the side or 2 to the
//side and 1 up. This is used in the for
// each loop to make my code more
//unnecessarily compact
// Copious amounts of for each loops to loop through all possible moves of
//the
// knight
for (int sign1 : signs) {
for (int sign2 : signs) {
...
//Makes sure the location is within the
//
//bounds of the board
controlledSquares.add(board[newY][newX]); // Adds valid
//square to the return array
}
}
}
}
...
// Steals list of squares that are on the board that the knight can move to
//from
// the previous method
